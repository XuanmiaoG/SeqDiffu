...
if __name__ == '__main__':
    ...
    # New parameter for multi-scale diffusion
    parser.add_argument('--num_scales', type=int, default=1, help='number of scales in multi-scale diffusion')
    ...
    Model_FILE = f"diffuseq_{args.dataset}_h{args.hidden_dim}_lr{args.lr}" \
                ...
                f"_seed{args.seed}_scales{args.num_scales}"  # Include scales in naming
    if args.notes:
    ...
    COMMANDLINE = f" OPENAI_LOGDIR={Model_FILE}  " \
                  ...
                  f"--hidden_dim {args.hidden_dim} --num_scales {args.num_scales} " \
