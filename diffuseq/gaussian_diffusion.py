...
# New Multi-Scale Diffusion Wrapper Class
class MultiScaleGaussianDiffusion(GaussianDiffusion):
    """
    Extension of GaussianDiffusion to enable multi-scale sequence generation.

    :param word_betas: Beta schedule for word-level granularity.
    :param phrase_betas: Beta schedule for phrase-level granularity.
    :param sentence_betas: Beta schedule for sentence-level granularity.
    """
    def __init__(
        self,
        word_betas,
        ...
        **kwargs
        ...
        self.word_diffusion = GaussianDiffusion(betas=word_betas, **kwargs)
        self.phrase_diffusion = GaussianDiffusion(betas=phrase_betas, **kwargs)
        self.sentence_diffusion = GaussianDiffusion(betas=sentence_betas, **kwargs)

    def training_losses_multi_scale(self, model, *args, **kwargs):
        """
        Compute training losses across all scales and combine them.
        """
        word_loss = self.word_diffusion.training_losses(model, *args, **kwargs)
        phrase_loss = self.phrase_diffusion.training_losses(model, *args, **kwargs)
        sentence_loss = self.sentence_diffusion.training_losses(model, *args, **kwargs)

        # Combine losses, adjusting weights as needed.
        combined_loss = (
            0.5 * word_loss["loss"] + 0.3 * phrase_loss["loss"] + 0.2 * sentence_loss["loss"]
        )

        return {"loss": combined_loss, "word_loss": word_loss, "phrase_loss": phrase_loss, "sentence_loss": sentence_loss}

    def p_sample_multi_scale(self, model, *args, **kwargs):
        """
        Perform hierarchical sampling across all scales.
        """
        word_sample = self.word_diffusion.p_sample(model, *args, **kwargs)
        phrase_sample = self.phrase_diffusion.p_sample(model, *args, **kwargs)
        sentence_sample = self.sentence_diffusion.p_sample(model, *args, **kwargs)

        return {
            "word_sample": word_sample,
            "phrase_sample": phrase_sample,
            "sentence_sample": sentence_sample,
        }

    def p_sample_loop_multi_scale(self, model, shape, *args, **kwargs):
        """
        Generate samples from the model using hierarchical sampling.
        """
        word_samples = self.word_diffusion.p_sample_loop(model, shape, *args, **kwargs)
        phrase_samples = self.phrase_diffusion.p_sample_loop(model, shape, *args, **kwargs)
        sentence_samples = self.sentence_diffusion.p_sample_loop(model, shape, *args, **kwargs)
...
# Integration of Multi-Scale Components
# Ensure compatibility with other functions and the original GaussianDiffusion API.

# Keep original classes and methods intact to maintain functionality.

"""
This modification extends GaussianDiffusion with hierarchical multi-scale diffusion.
The original components are preserved for compatibility, enabling both standard and advanced workflows.
"""
