...
class TrainLoop:
    class TrainLoop:
        #更改TrainLoop类，使其支持多尺度
        def __init__(
            self,
            *,
            model,
            diffusion,  # Single diffusion process (optional)
            multi_scale_diffusions,  # List of diffusion processes for different scales
            data,
            batch_size,
            microbatch,
            lr,
            ...
            self.multi_scale_diffusions = multi_scale_diffusions  # Added for multi-scale diffusion
            self.data = data
    ...
    def run_loop(self):
        ...
        if (self.step - 1) % self.save_interval != 0:
            self.save()
    # 更改run_step函数，使其支持多尺度
    def run_step(self, batch, cond):
        """
        Run one training step for all scales in multi-scale diffusion.
        """
        for scale, diffusion in enumerate(self.multi_scale_diffusions):
            self.forward_backward(batch, cond, scale, diffusion)
            if self.use_fp16:
                self.optimize_fp16()
            else:
    ...
    # 更改forward_backward函数，使其支持多尺度
    def forward_backward(self, batch, cond, scale, diffusion):
        """
        Perform forward and backward propagation for a specific scale.
        """
        zero_grad(self.model_params)
        for i in range(0, batch.shape[0], self.microbatch):
            micro = batch[i : i + self.microbatch].to(dist_util.dev())
            ...
            compute_losses = functools.partial(
                diffusion.training_losses,  # Use the scale-specific diffusion
                self.ddp_model,
                micro,
                t,
                model_kwargs=micro_cond,
            ...
            log_loss_dict(
                diffusion, t, {f"scale_{scale}_{k}": v * weights for k, v in losses.items()}
            )
    ...
    def save(self):
        """
        Save model and EMA parameters for each scale.
        """
        def save_checkpoint(rate, params, scale=None):
            state_dict = self._master_params_to_state_dict(params)
            if dist.get_rank() == 0:
                logger.log(f"saving model {rate} at scale {scale}...")
                filename = (
                    f"scale_{scale}_model{(self.step + self.resume_step):06d}.pt"
        ...
        for scale, diffusion in enumerate(self.multi_scale_diffusions):
            for rate, params in zip(self.ema_rate, self.ema_params):
                save_checkpoint(rate, params, scale=scale)
